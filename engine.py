...
class DraftIQEngine:
    ...
    def _match_player_name(self, name):
        ...
        return None  # zero or multiple (ambiguous) matches
    ...
    def next_pick_probability(self, player):
        ...
        if pd.isna(adp):
            return 50
        gap = adp - next_pick
        # Scale the swing from 50% by the gap relative to picks_away, not a
        # fixed ADP-point slope: an ADP gap of a given size means something
        # very different depending on how many actual picks happen before
        # your next turn (1 vs 20+). Using picks_away as the scale keeps
        # the well-before/around/well-after gradient meaningful in both a
        # short wait (small gap -> near-certain) and a long wait (needs a
        # proportionally larger gap to be near-certain).
        probability = 50 + (gap / max(picks_away, 1)) * 45
    ...
    def draftiq_decision_engine(self, count=10):
        ...
        for _, player in available.iterrows():
            ...
            decision_score = (
                score * 0.45 + player_urgency * 0.25 + risk_score * 0.20
        ...
        # Labels are assigned using score thresholds, but "DRAFT THIS" is
        # additionally capped to the top 3 players BY RANK, even if more
        # than 3 meet the score threshold. Early in a deep draft it's
        # common for several genuinely-strong tier-1 players to all clear
        # 78+ simultaneously (nothing wrong with any individual score) -
        # but flagging 6-7 players at once as "the one to draft right now"
        # defeats the point of the label. The rank cap keeps it meaningful
        # without changing what counts as objectively a good score.
        #
        # The old version also gated "DRAFT THIS" on tier == 1 directly,
        # separately from the score threshold. Since tier's influence is
        # already folded into the score itself (a tier bonus is added
        # before this point), that extra gate was redundant - and worse,
        # it could let a lower-scoring tier-1 player outrank a higher-
        # scoring non-tier-1 player in label terms, even though the score
        # (and therefore the sort order) said the opposite. Scoring alone
        # (plus the rank cap) keeps labels strictly consistent with the
        # actual ranking.
        DRAFT_THIS_RANK_CAP = 3
        labels = []
        for rank_position, (_, row) in enumerate(results_df.iterrows(), start=1):
            score = row["Decision Score"]
            if score >= 78 and rank_position <= DRAFT_THIS_RANK_CAP:
