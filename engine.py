...
class DraftIQEngine:
    ...
    def get_roster_slots(self, team_number=None):
        ...
        currently configured (including via Sleeper sync), not a
        hardcoded roster shape. Empty slots return player=None so the UI
        can render them as open."""
        ...
        for pos in SLOT_ORDER:
            ...
            for i in range(count):
                ...
                if pos == "FLEX":
                    ...
                    slots.append({"slot": label, "position": filled_pos or "FLEX", "player": name})
                else:
                    name, filled_pos = take({pos})
                    slots.append({"slot": label, "position": pos, "player": name})
        ...
        bench = [{"player": n, "position": p} for n, p in remaining]
    ...
    def _match_player_name(self, first_name, last_name, position=None, team=None):
        ...
        # "(POS)" disambiguator suffix (e.g. "J. Love (RB)"), then take
        # everything after the first token (the abbreviated first name) as
        # the last-name portion.
        stripped = stored_lower.str.replace(r"\s*\([a-z]+\)$", "", regex=True)
        last_portion = stripped.str.split().str[1:].str.join(" ")
        ...
        def _resolve(matches):
            ...
            if len(matches) == 0:
                return None
            # More than one candidate - narrow using position/team from
            # Sleeper's own pick metadata rather than guessing.
            narrowed = matches
            if position:
            ...
            if team:
                ...
                if len(by_team) > 0:
                    narrowed = by_team
            return narrowed.iloc[0]["name"] if len(narrowed) == 1 else None

        # Exact match on the last-name portion first: "Robinson" is a
        # substring of "Robinson Jr.", but they aren't the same last name,
        # so an exact-match tier resolves that without even needing
        # position/team. Only fall back to loose substring containment
        # (the pre-existing behavior) when nothing matches exactly - e.g.
        # if Sleeper's own suffix formatting doesn't line up with the
        # stored name's.
        exact_last = self.players_df[last_portion == last_l]
        if len(exact_last) > 0:
            return _resolve(exact_last)

        contains_last = self.players_df[stripped.str.contains(last_l, regex=False)]
        return _resolve(contains_last)
    ...
    def calculate_draftiq_score(self, player, tier_table=None, needs=None,
        ...
        # ADP disagreeing with Rank is actually useful information. This
        # also means a Sleeper-imported pool (where ADP is set equal to
        # Rank, since Sleeper doesn't expose real ADP) no longer double-
        # votes on quality - ADP's only remaining job, timing, still
        # works fine even when it's rank-derived.
        ...
        # heavily-weighted buckets of Decision Score at once.
    ...
    def draftiq_decision_engine(self, count=10):
        ...
        for _, player in available.iterrows():
            ...
            results.append({
                ...
                "Player Urgency": min(100, round(player_urgency)), "Position Urgency": position_urgency,
